...
void main()
{
	...
	vec3 brume_color = vec3(0, 102, 204) / 256.0f;
	float dmax = 1000.0f;
	float alpha = min(1, length(camera_position - fragment.position)/dmax);

  	FragColor = vec4((1-alpha)*color_shading + alpha*brume_color, material.alpha * color_image_texture.a);
